...
async function loadAll() {
  const [posts, refs, schedules, config, scanResults, analytics] = await Promise.all([
    api('/api/posts'), api('/api/references'), api('/api/schedules'),
    api('/api/config'), api('/api/scan/results'), api('/api/analytics')
  ]);
  Object.assign(state, { posts, references: refs, schedules, config, scanResults, analytics });
}
  ...
  openModal(`Posts for ${formatDate(dateStr)}`, body => {
    ...
    posts.forEach(p => {
      ...
      card.innerHTML = `
        ...
        <div class="fw-600">${p.title || 'Untitled'}</div>
        <div class="text-sm text-muted mt-12">${p.content.slice(0, 200)}</div>
        <div class="flex gap-8 mt-12">
          <button class="btn btn-sm btn-secondary" onclick="closeModal();editPost('${p.id}')">Edit</button>
...
function renderCreate(container) {
  ...
  contentEl.addEventListener('input', () => { updateCharCount(); updatePreview(); });
  ...
  document.getElementById('btn-save-draft').onclick = async () => {
    ...
    if (isEdit) { await api(`/api/posts/${p.id}`, { method: 'PUT', body: data }); state.editingPost = null; }
    else await api('/api/posts', { method: 'POST', body: data });
    await loadAll();
    location.hash = 'dashboard';
  };
  document.getElementById('btn-schedule').onclick = async () => {
    const data = gatherPostData('scheduled', hashtags);
    if (!data.scheduledAt) { alert('Please select a schedule date'); return; }
    if (isEdit) { await api(`/api/posts/${p.id}`, { method: 'PUT', body: data }); state.editingPost = null; }
    else {
...
function renderInspiration(container) {
  ...
  function renderItems(filter) {
    ...
    if (!items.length) {
      grid.innerHTML = '<div class="empty-state" style="grid-column:1/-1"><h3>No inspiration content yet</h3><p>Run a scan to discover trending content</p></div>';
      return;
    }
    items.slice(0, 20).forEach(item => {
      ...
      card.innerHTML = `
        ...
        <div class="insp-actions">
          ...
          <button class="btn btn-sm btn-ghost remix-btn">Remix</button>
        </div>`;
    ...
    openModal('Scan for Content', body => {
      ...
      document.getElementById('scan-go').onclick = async () => {
        ...
        try {
          ...
          document.getElementById('scan-status').textContent = res.success ? 'Scan started. Results will appear in a few minutes.' : 'Error: ' + (res.error || 'Unknown');
...
function renderReferences(container) {
  ...
  renderRefs('', '');
  document.getElementById('ref-search').oninput = () => renderRefs(document.getElementById('ref-search').value, document.getElementById('ref-platform-filter').value);
...
function renderAnalytics(container) {
  ...
  const maxImpressions = Math.max(1, ...trend.map(t => t.impressions));

  container.innerHTML = `
    <div class="page-header"><div><h1>Analytics</h1><div class="subtitle">Track your content performance</div></div></div>
    <div class="card-grid card-grid-3 mb-20">
      <div class="card stat-card"><div class="stat-value">${(eng.totalImpressions || 0).toLocaleString()}</div><div class="stat-label">Total Impressions</div></div>
      <div class="card stat-card"><div class="stat-value">${(eng.totalEngagement || 0).toLocaleString()}</div><div class="stat-label">Total Engagement</div></div>
      <div class="card stat-card"><div class="stat-value">${eng.avgEngagementRate || 0}%</div><div class="stat-label">Avg Engagement Rate</div></div>
    </div>
    <div class="card-grid card-grid-2">
      <div class="card">
        <h3 class="fw-600 mb-16">Weekly Impressions</h3>
        <div class="chart-bars" id="weekly-chart"></div>
      </div>
      <div class="card">
        <h3 class="fw-600 mb-16">Platform Breakdown</h3>
  ...
  trend.forEach(t => {
    const pct = (t.impressions / maxImpressions) * 100;
    weeklyChart.innerHTML += `<div class="chart-bar-wrap"><div class="chart-bar-value">${t.impressions}</div><div class="chart-bar" style="height:${pct}%"></div><div class="chart-bar-label">${t.day}</div></div>`;
  });

  const platChart = document.getElementById('platform-chart');
  const maxPlatImpressions = Math.max(1, ...Object.values(platforms).map(p => p.impressions));
  Object.entries(platforms).forEach(([plat, data]) => {
    const pct = (data.impressions / maxPlatImpressions) * 100;
    platChart.innerHTML += `<div class="chart-bar-wrap"><div class="chart-bar-value">${data.impressions}</div><div class="chart-bar ${plat}" style="height:${pct}%"></div><div class="chart-bar-label">${PLATFORM_LABELS[plat] || plat}</div></div>`;
  });

  if (!Object.keys(platforms).length) {
    platChart.innerHTML = '<div class="empty-state" style="height:100%;display:flex;align-items:center;justify-content:center"><p class="text-muted">Publish posts to see platform data</p></div>';
  }
}
